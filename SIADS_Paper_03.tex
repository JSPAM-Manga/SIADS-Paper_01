...
and
\begin{equation}
	RMSE^2 = \dfrac{1}{|A(X) \cap A(X_0)|}\sum_{i,j = 0}^{n_{bin}-1} w_{i,j}(X)|v_{i,j}(X)-v_{i,j}(X_0)|^2
\end{equation}
...
In these equations, $X$ and $X_0$ are parameter strings that refer to the \textit{simulation} and \textit{target}, respectively.  The sets $A(X)$ and $A(X_0)$ contain all the bins that have particles present, and their cardinalities equal the number of bins in these sets.  Therefore,  $|A(X) \cap A(X_0)|$
equals the number of \textit{overlapping} bins they share. The overlap fraction, $OvrFrac$, is the fraction of bins in which both the simulation and the target have particles present. $RMSE$ is the weighted root-mean-squared error between the velocity fields only calculated over the overlapping bins and normalized by the number of overlapping bins (thus, $w_{i,j} = 1$ if the pixels overlap, $w_{i,j} = 0$ otherwise).

We experimented with different weighting schemes...

Although this weighting system is not perfect, it works adequately for systems where was have a measured velocity across the interacting galaxy system and is consistent with the type of data available
from MaNGA.
...
\begin{gather}
	...
	m_j = (1-b)^{(j+1)/3}
\end{gather}
and $0<a,b<1$. These two factors serve to alternate between increasing, decreasing, and resetting the $\sigma$ values so that the MCMC can escape local minima and fall into a thin global minimum quicker. We will discuss the values used for $a$ and $b$ in the next section, along with the impact this modification has on convergence.
...
Since MCMC methods are stochastic, individual runs tell us nothing about their convergence. We must run many tests and calculate statistics on the collective results. Therefore, our technique for testing consisted of running many instances of the MCMC, all with the same target and initial perturbation and calculating the percentage which converged (reached a minimum error threshold) so as to calculate an approximation to the convergence probability. This was done on a univariate basis by choosing an interval of evenly-spaced points centered on the target parameter value. 
The interval widths used in our analysis were created by analyzing the best models from the {\it Galaxy Zoo: Mergers} project.  We took the top $25\%$ of the models for the target system and found the standard deviation for each parameter.   We then doubled the standard deviations for each these parameters and used them as our interval widths.  Since the top $25\%$ models from {\it Galaxy Zoo: Mergers} represent a human-determined consensus of reasonably good fits to the morphology of the system, the interval widths we used are likely to produce models that are morphologically similar to our target galaxy (even if the parameters were not actually in the Galaxy Zoo: Mergers set).
These points were then repeatedly used as the initial perturbation for the MCMC algorithm. By calculating this probability and plotting it over the interval, we can visually inspect how much the shape of the error curve impacts the rate of convergence, as well as compare the convergence of different parameters.  


%The interval's width was chosen to be equal to the twice the standard deviation of top $25\%$ runs' value of that parameter taken from the ranked states in the Merger Zoo Project.  These top $25\%$ runs represent a human-determined consensus of reasonably good fits to the morphology of the system.
...
We began with a univariate analysis of the error function, sampling it over an interval of equally-spaced points centered on the target's parameter value.
...
Graham  - write this - picking the optimal and first step size for the MCMC
...
\begin{figure}[!h]
	...
	\begin{align*}
		...
		\includegraphics[scale = 0.25]{Error-radPrim} & \includegraphics[scale = 0.25]{Error-radSec} \\
	\end{align*}
	\caption{Plots.}
\end{figure}
...
We will first consider the convergence using MCMC of a each parameter individually while holding the other values fixed.  
...
To measure the sensitivity, we sweep through a range of physically reasonable values for each the the parameters and calculate the fitness as a function fo the parameter's value.  The peak of the fitness function should occur when we have a perfect match.  
 ...
 \subsection{Parameter values and fitness}
 
 Plots of the error vs parameter values for each parameter.  
 
 
 \subsection{Convergence and parameter values}
