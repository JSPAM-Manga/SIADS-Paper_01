...
\subsection{Measuring Convergence}
...
\subsubsection{Error vs. Parameters}
...
\subsubsection{Probablity of Convergence within $n$-Steps}


\section{Results}
Since MCMC is stochastic, individual runs tell us nothing about its convergence. We must run many tests and calculate statistics on the collective results. Therefore, our method of testing consisted of running many instances of the MCMC in parallel, all with the same target and initial parameters, but allowing different paths through parameter space. We let the MCMC run for a fixed number of steps and determined if a run converged based on whether it reached a specified error threshold. We would then generate numerous plots and statistics with which we could determine how successful the runs were.
