...
\title{Parameter Estimation for JSPAM Galactic Merger Simulations}
...
\begin{figure}[!h]
	...
	\end{align*}
	\caption{Visible image and gas $z$-velocity of a merger observed by MaNGA.}
\end{figure}

\section{Methods}

The two fundamental parts to the solution of an optimization problem such as ours are 1) a reliable fitness function and 2) a robust optimization algorithm. In this section, we will examine the reasoning behind the function we developed as well as the already-existing algorithm we modified.

\subsection{Fitness Function}

There are essentially two types of information which our fitness function may draw upon: particle positions and velocities. Obviously, however, we can't evalute our fitness function on a per-particle basis because--among a great many reasons--once we begin using real merger targets, we will not have that information for the actual stars. It was very important that we kept the context of real mergers in our minds as we experimented with the different fitness functions because whatever we decided on in the testing phase must still be able to work when applied to real targets. Fortunately, this restriction led us to an elegant solution to the problem. Since the data format for the real targets is simply a $54 \times 54$ image for the $z$-velocity and a $512 \times 512$ image for visible light, simply binning the particle number and $z$-velocity will give us an equivalent format for our simulations.
...
\begin{figure}[!h]
	...
	\includegraphics[scale = 0.3]{Plot_GalaxyVelocity_Binned.jpg}
		\caption{Left: an image of data points during a merger simulation. Right: a 50x50 square-binning of the left image (high-density regions are red and low-density regions are blue). Bottom: a 50x50 square-binning of the $z$-velocities.}
\end{figure}

Having binned the simulation, we have two grids representing the $x$-$y$ distribution of particle density and $z$-velocity. We now divide the fitness function into two parts, one relateing to the particle distribution and the other to the RMS error of the $z$-velocities of two mergers:
\begin{equation}
	f(sim, tar) = (1-OvrFrac) \cdot RMS
\end{equation}
where,
\begin{equation}
	OvrFrac = \dfrac{A_{ovr}}{A_{sim}+A_{tar}-A_{ovr}}
\end{equation}
and
\begin{equation}
	RMS^2 = \sum_{k}^{} w_k(v^{sim}_k-v^{tar}_k)^2
\end{equation}
Note that our fitness function is an error function, so small values mean small error (high fitness). In these equations, $sim$ and $tar$ refer to the density or $z$-velocity fields of the merger \textit{simulation} and the merger \textit{target}, respectively. $OvrFrac$ is the fraction of area/bins/pixels in which both the simulation and the target have particles present. RMS is the root-mean-squared error between the velocity fields only calculated over the overlapping pixels (thus, $w_k = 1$ if the pixels overlap, $w_k = 0$ otherwise).
