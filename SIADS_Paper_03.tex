...
In the original {\it Galaxy Zoo: Mergers} project, the center of mass positions of the two galaxies was measured directly from astronomical images.  This measurement provides on of the constraints on our models.  The centroids of our simulations will always align with the galaxies.
...
\end{gather}
where $n_{bin}$ is the number of bins on one axis, $n_{pts}$ is the total number of particles (included for normalization purposes), and $dx, dy, dz$ are the dimensions of a single bin.  We have aalso moved from a continuous representation of the system using $x, y, z$ to using the indexes of the bin at  $i, j, k$.  Thus the variable $v_{i,j,k}$ is the mean velocity of the particles in the bin at $x_i, y_j, z_k$ and $n_{ijk}$ is the number of particles within this bin.
