...
#	param_toFit = [ 10, 11, 12, 13 ]
	...
	for j in range(nGen):
		...
		data2.append([])
		data2[j] = param_start[0:num_param]
	...
	a = np.corrcoef( data2[:,-3], data3[:,-3] )[1,0]
	b = np.corrcoef( data2[:,-2], data3[:,-2] )[1,0]
	c = np.corrcoef( data2[:,-1], data3[:,-1] )[1,0]
	
	print "ovrfrac corr: ", str(a)
	print "rmse corr: ", str(b)
	print "error corr: ", str(c)
