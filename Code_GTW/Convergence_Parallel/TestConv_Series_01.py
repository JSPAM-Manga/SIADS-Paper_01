...
def main():
	...
	param_real[6] = a
	binC_real, binV_real, binCu_real, binVu_real = solve(param_real, nBin, bound, alpha)
	...
	for j in range(nGen):
		...
		param_start[ind] = param_real[ind] + (j*2.0 - nGen)/nGen/2.0*width
		
		binC, binV, binCu, binVu = solve(param_start, nBin, bound, alpha)
		W = GetWeights( nBin, binC, binCu, weights )
		error, RMSE, OvrFrac = ErrorFunction2D( nBin, binV, binV_real, W )
...
def BinField(nBin, bound, RV, alpha):
