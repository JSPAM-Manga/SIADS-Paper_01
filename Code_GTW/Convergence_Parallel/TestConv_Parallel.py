...
def main():
	...
	redosRemain = redos
	paraReal  = 6
	times = int(math.ceil((1.0*redos)/paraReal))
	shift = 0
	num_param = 14
...
#		print metOutFile
		...
		for k in range(times):
			...
			cmnd = "./test.o " + metOutFile.strip() + " " + str(para) + " " + paramTarg_str.strip() + " " + paramStart_str.strip() + " " + str(paraLast+shift)# + " > out.txt"
#			print cmnd
			call(cmnd, shell=True)
			paraLast += para
		# end
		
		for kk in range(redos):
			k = kk + shift
			if( k < 10 ):
...
#			print metOutFile_2
			...
			conv[j] += GetConv(metOutFile_2, nStep)
		# end
		conv[j] /= (redos*1.0)
